          ...
          <el-table-column label="Actions" width="180" fixed="right">
            <template #default="{ row }">
              <div class="action-buttons">
                <el-button type="primary" size="small" @click="editWorkTime(row)">
                  <el-icon><Clock /></el-icon>
                  Temps
                </el-button>
                <el-button type="success" size="small" @click="openPretPersonnel(row)">
        ...
        <div class="livre-paie-content">
          ...
          <div class="livre-paie-table-container">
            <el-table :data="livrePaieData" stripe max-height="600">
              ...
              <el-table-column label="Nbre de jrs trv mois" prop="nbJoursTravail" width="170" align="center" />
              ...
              <el-table-column label="Regularisation" prop="netRegulPayer" width="150" align="right">
                <template #default="{ row }">
        ...
        <div class="bulletins-content">
          ...
          <div class="bulletins-table-container">
            <el-table :data="filteredBulletins" :loading="bulletinsLoading" stripe max-height="600">
              ...
              <el-table-column label="Nom & Prénoms" prop="nomComplet" min-width="200" />
              <el-table-column label="Nbre de jrs trv mois" prop="nbJoursTravail" width="170" align="center" />
              <el-table-column label="Statut" prop="statutSpecial" width="150" />
              <el-table-column label="Renumeration" prop="remunerationSpeciale" width="160" align="right">
                <template #default="{ row }">
                  {{ formatCurrency(row.remunerationSpeciale) }}
                </template>
              </el-table-column>
              <el-table-column label="Avance & Acompte" prop="avceAcpte" width="150" align="right">
                <template #default="{ row }">
              ...
              <el-table-column label="Regularisation" prop="regularisation" width="150" align="right">
                <template #default="{ row }">
    ...
    >
      <template #header>
        <div class="mm-header">
          ...
          <el-button link class="mm-close" @click="closePretModal">
            <el-icon :size="20"><Close /></el-icon>
          </el-button>
        </div>
      </template>
      ...
      <div class="mm-body">
        <el-form :model="loanForm" :rules="loanRules" ref="loanFormRef" label-position="top" size="large">
          <el-form-item label="Agent" prop="personnelId" required>
            <el-input :model-value="selectedPersonnel ? `${selectedPersonnel.nomComplet} - ${selectedPersonnel.matricule}` : ''" disabled />
          </el-form-item>
          ...
          <el-form-item label="Montant" prop="montant" required>
            <el-input-number
              ...
              :formatter="(value: number) => `${(value || 0).toLocaleString()} XOF`"
          ...
          <el-form-item label="Période Début Prélèvement" prop="periodeDebut" required>
            <el-select
              ...
              filterable
              clearable
              :loading="loanPeriodesLoading"
            >
              <el-option
                v-for="periode in loanPeriodes"
                :key="periode.value"
                :label="periode.label"
                :value="periode.value"
              />
          ...
          <el-form-item label="Motif" prop="motif">
            <el-input v-model="loanForm.motif" type="textarea" :rows="3" placeholder="Motif du prêt" />
          </el-form-item>
      ...
      <template #footer>
        <div class="mm-footer">
          <el-button size="large" plain @click="closePretModal">Annuler</el-button>
          <el-button type="success" size="large" :loading="savingLoan" @click="saveLoan">
            Enregistrer
          </el-button>
        </div>
      </template>
...
import { livrePaieSpecialeService as livrePaieService, situationMatriToCode, type PeriodePaieRef } from '@/services/livrepaie-speciale.service'
import {
...
import { pretspersonnelsService, type PretPersonnelRequest } from '@/services/pretspersonnels.service'
...
// Données réactives
const isSpecialAgentsLivre = computed(() => true)
...
const loanPeriodes = ref<Array<{ value: string; label: string }>>([])
const loanPeriodesLoading = ref(false)
...
const mapLivreRow = (r: any): Bulletin => {
  const b = r?.bullpaie || {}
  const personnel = r?.contratPersonnel?.personnel || r?.contratPersonnel?.employee || r?.personnel || r?.employee || {}
...
const mapBulletinRow = (r: any): Bulletin => {
  const personnel = r?.contratPersonnel?.personnel || r?.contratPersonnel?.employee || r?.personnel || r?.employee || {}
  const specialContract = r?.specialcontract || r?.specialContract || r?.contratSpecial || r?.contratSpecifique || r?.contract || {}
  const matricule = personnel.matricule || r.matricule || ''
  const nomComplet = personnel.nomComplet
    || `${personnel.prenom || ''} ${personnel.nom || ''}`.trim()
    || r.nomComplet
  ...
  const regularisation = Number(r.regularisation ?? r.mtregularisation ?? r.netRegulPayer ?? r.netRegulpayer ?? 0)
  const netRegulPayer = Number(r.netRegulPayer ?? r.netRegulpayer ?? 0)
  const netAPayer = Number(
    r.netPayer
    ?? r.netapayer
    ?? r.netAPayer
    ?? r.bulletinSpeciale?.netPayer
    ?? r.bulletinSpeciale?.netapayer
    ?? r.bulletinSpeciale?.netAPayer
    ?? 0
  )
  ...
  const mapped = {
    ...
    nomComplet,
    statut: personnel.enSommeil ? 'sommeil' : 'actif',
    statutSpecial: specialContract.typeContrat
      ?? specialContract.typecontrat
      ?? specialContract.type
      ?? r.typeContrat
      ?? r.typecontrat
      ?? r.statutSpecial
      ?? '',
    remunerationSpeciale: Number(
      specialContract.remunerationForfaitaire
      ?? specialContract.renumeration
      ?? specialContract.remuneration
      ?? specialContract.netpayer
      ?? r.remunerationForfaitaire
      ?? r.renumeration
      ?? r.remuneration
      ?? r.netpayer
      ?? 0
    ...
    nbJoursTravail: Number(
      r.jourTravail
      ?? r.jourtravail
      ?? r.bulletinSpeciale?.jourTravail
      ?? r.bulletinSpeciale?.jourtravail
      ?? 30
    ),
...
const exportRowsToExcel = (
  ...
  const exportData = rows.map((row) => {
    return {
      ...
      'Nbre de jrs trv mois': numericValue(row.nbJoursTravail),
      'Avance & Acompte': numericValue(row.avceAcpte),
      Pret: numericValue(row.pretAlios),
      Regularisation: numericValue(row.netRegulPayer),
      'Net à Payer': numericValue(row.netAPayer),
      'Total Masse Salariale': numericValue(row.totalMasseSalariale)
    }
  })
...
const toBackendDateFr = (date: string) => {
  if (!date) return ''
...
const loadLoanPeriodes = async () => {
  loanPeriodesLoading.value = true
  try {
    const data = await pretspersonnelsService.getPeriodesPaie()
    loanPeriodes.value = data.map((p: any) => ({
      value: String(p.id),
      label: p.affiche || p.libelle || p.periode || `Période ${p.id}`
    }))
  } catch (error) {
    console.error('Erreur chargement périodes prêt:', error)
    ElMessage.error('Erreur lors du chargement des périodes de paie')
    loanPeriodes.value = []
  } finally {
    loanPeriodesLoading.value = false
  }
}

const resetLoanForm = () => {
  Object.assign(loanForm, {
    personnelId: selectedPersonnel.value?.id || 0,
    type: '',
    montant: 0,
    dateContraction: '',
    periodeDebut: '',
...
const openPretPersonnel = async (personnel: Personnel) => {
  selectedPersonnel.value = personnel
  resetLoanForm()
  loanForm.personnelId = personnel.id
  showPretModal.value = true
  if (loanPeriodes.value.length === 0) {
    await loadLoanPeriodes()
  }
}

const closePretModal = () => {
  showPretModal.value = false
  resetLoanForm()
}
...
const saveLoan = async () => {
  ...
  try {
    ...
    const request: PretPersonnelRequest = {
      montant: Number(loanForm.montant) || 0,
      echelonage: Number(loanForm.duree) || 1,
      pret: parseInt(String(loanForm.type), 10),
      idpers: Number(loanForm.personnelId),
      dEmprunt: toBackendDateFr(loanForm.dateContraction),
      periodepaie: periodeId
    }

    await pretspersonnelsService.savePretEmployee(request)
