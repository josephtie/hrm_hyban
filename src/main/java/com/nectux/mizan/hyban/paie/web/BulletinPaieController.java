...
//	}
    ...
    public @ResponseBody List<LivreDePaie> calculalenversListe(
        ...
        for (ContratPersonnel contrat : contratsActifs) {

            try {

                BigDecimal netAPayer = safe(contrat.getNetAPayer());

                if (netAPayer.compareTo(BigDecimal.ZERO) == 0) {
                    logger.warn("⚠ Net à payer manquant pour {}", contrat.getPersonnel().getNom());
                    continue;
                }

                logger.info("➡ Traitement {} | Net cible: {}",
                        contrat.getPersonnel().getNom(),
                        netAPayer);

                // 🔹 Ancienneté
                long annees = ChronoUnit.YEARS.between(
                        contrat.getPersonnel().getDateArrivee()
                                .toInstant()
                ...
                long anciennete = annees +contrat.getAncienneteInitial();




                int anc = (int) anciennete;
                int op = (anc < 2) ? 0 : Math.min(anc, 25);

                Float nbpart = calculNbrepart(
                        contrat.getPersonnel().getNombrEnfant(),
                        contrat.getPersonnel()
                );

                // 🔹 Récup primes
                List<PrimePersonnel> primes = primePersonnelRepository
                        .findByContratPersonnelPersonnelIdAndPeriodePaieId(
                ...
                List<PrimePersonnel> mutuelle = new ArrayList<>();
                List<PrimePersonnel> sociale = new ArrayList<>();
                List<PrimePersonnel> gains = new ArrayList<>();

                for (PrimePersonnel p : safeList(primes)) {
                    ...
                    switch (p.getPrime().getEtatImposition()) {
                        ...
                        case 4 -> mutuelle.add(p);
                        case 5 -> gains.add(p);
                        case 6 -> sociale.add(p);
                    }
                }

                // 🔥 INITIALISATION
                LivreDePaie livre = new LivreDePaie(
                        contrat.getPersonnel().getMatricule(),
                        contrat.getPersonnel().getNom() + " " + contrat.getPersonnel().getPrenom(),
                        nbpart,
                        op,
                        contrat.getCategorie().getSalaireDeBase(),
                        BigDecimal.valueOf(5000),
                        contrat.getIndemniteLogement(),
                        BigDecimal.ZERO,
                        BigDecimal.ZERO,
                        contrat,
                        null,
                ...
                // 🔥 ALGO INVERSÉ (BigDecimal)
                for (int i = 0; i < 20; i++) {

                    if (livre.getNetPayer().compareTo(BigDecimal.ZERO) == 0) {
                        throw new IllegalStateException("Net calculé = 0 → division impossible");
                    }

                    BigDecimal nouveauBrut = netAPayer
                            .multiply(livre.getBrutImposable())
                            .divide(livre.getNetPayer(), 2, RoundingMode.HALF_UP)
                            .setScale(0, RoundingMode.HALF_UP);

                    BigDecimal diff = nouveauBrut.subtract(livre.getBrutImposable());

                    BigDecimal nouveauSursalaire = livre.getSursalaire().add(diff);

                    livre = new LivreDePaie(
                            contrat.getPersonnel().getMatricule(),
                            contrat.getPersonnel().getNom() + " " + contrat.getPersonnel().getPrenom(),
                            nbpart,
                            op,
                ...
                // 🔹 Sauvegarde
                contrat.setSursalaire(livre.getSursalaire());
                contratPersonnelRepository.save(contrat);

                resultats.add(livre);

                logger.info("✅ OK {} | Net: {} | Brut: {}",
                        contrat.getPersonnel().getNom(),
                        livre.getNetPayer(),
                        livre.getBrutImposable());

            } catch (Exception e) {

                logger.error("❌ Erreur pour {} : {}",
                        contrat.getPersonnel().getNom(),
                        e.getMessage(),
                        e);
            }
    ...
    public @ResponseBody LivreDePaie calculalenvers(
            @RequestParam(value="idPersonnel", required=true) Long id,
            @RequestParam(value="netApayer", required=true) Double netCible,
            Principal principal) {

        ContratPersonnel contrat = contratPersonnelRepository.findByPersonnelIdAndStatut(id, true);
        ...
        long anneesAnciennete = ChronoUnit.YEARS.between(
                contrat.getPersonnel().getDateArrivee().toInstant()
                        .atZone(ZoneId.systemDefault())
                        .toLocalDate(),
                LocalDate.now()
        );

        long anciennete = anneesAnciennete +contrat.getAncienneteInitial();

        int op = (anciennete < 2) ? 0 : (anciennete <= 25 ? (int) anciennete : 25);
        ...
        // Primes
        // =========================
        List<PrimePersonnel> listIndemnite = primePersonnelRepository
                .findByContratPersonnelPersonnelIdAndPeriodePaieId(
                        contrat.getPersonnel().getId(),
                        maperiode.getId()
                );
        ...
        List<PrimePersonnel> nonBrut = new ArrayList<>();
        List<PrimePersonnel> mutuelle = new ArrayList<>();
        List<PrimePersonnel> social = new ArrayList<>();
        List<PrimePersonnel> net = new ArrayList<>();

        for (PrimePersonnel p : listIndemnite) {
            int type = p.getPrime().getEtatImposition();

            if (type == 1) brut.add(p);
            else if (type == 2) nonBrut.add(p);
            else if (type == 3) {
                if (p.getPrime().getMtExedent() != null) {
                    brut.add(p);
                    nonBrut.add(p);
                }
            } else if (type == 4) mutuelle.add(p);
            else if (type == 5) net.add(p);
            else if (type == 6) social.add(p);
        }

        // =========================
        // 🔥 CONVERGENCE NET → SURSALAIRE
        // =========================
        BigDecimal netCibleBD = BigDecimal.valueOf(netCible);
        BigDecimal tolerance = new BigDecimal("1");

        BigDecimal sursalaire = BigDecimal.valueOf(5000);
        LivreDePaie bulletin = null;

        for (int i = 0; i < 25; i++) {

            bulletin = new LivreDePaie(
                    contrat.getPersonnel().getMatricule(),
                    contrat.getPersonnel().getNom() + " " + contrat.getPersonnel().getPrenom(),
                    nbpart,
                    op,
            ...
            BigDecimal netCalcule = bulletin.getNetPayer();

            BigDecimal erreur = netCibleBD.subtract(netCalcule);

            if (erreur.abs().compareTo(tolerance) <= 0) {
                break; // ✅ convergence atteinte
            }

            // 🔥 correction progressive (stabilité)
            BigDecimal correction = erreur.multiply(new BigDecimal("0.5"));

            sursalaire = sursalaire.add(correction);
        }
...
//	}
	...
	public ResponseEntity<byte[]> exportBulletinsJulaya(HttpServletResponse response) throws IOException {
		...
		try (InputStream inputStream = templateFile.getInputStream();
			...
			for (BulletinPaie bulletin : bulletins) {
                ...
                BigDecimal net = Optional.ofNullable(bulletin.getNetapayer()).orElse(BigDecimal.ZERO);
                row.createCell(1).setCellValue(net.doubleValue());
	...
	public BulletinPaie getPayslipData(Long employeeId,BulletinPaie bulletin){
		...
		for(Rubrique rubrique : listPrimeImp1){
			...
			if(!listprimepersonnel1.isEmpty() &&listprimepersonnel1.get(0) !=null){
...
//			listImprimBulletinPaie.add(imprimBulletinPaieRESP);
				...
				ImprimBulletinPaie imprimBulletinPaiePF= new ImprimBulletinPaie();
				imprimBulletinPaiePF.setLibelle("PRESTATION FAMILIALE");
				imprimBulletinPaiePF.setTaux(BigDecimal.valueOf(5.75));
				...
				ImprimBulletinPaie imprimBulletinPaieAC= new ImprimBulletinPaie();
				imprimBulletinPaieAC.setLibelle("ACCIDENT DE TRAVAIL");
				imprimBulletinPaieAC.setTaux(BigDecimal.valueOf(2));
				imprimBulletinPaieAC.setBase(BigDecimal.valueOf(70000));
				imprimBulletinPaieAC.setRetenuePatron(bulletin.getAccidentTravail());
				listImprimBulletinPaie.add(imprimBulletinPaieAC);
				...
				ImprimBulletinPaie imprimBulletinPaieTA= new ImprimBulletinPaie();
				imprimBulletinPaieTA.setLibelle("FDFP -TAXE APPRENTISAGE ");
				imprimBulletinPaieTA.setTaux(BigDecimal.valueOf(0.4));
				imprimBulletinPaieTA.setBase(bulletin.getBrutImposable());
				imprimBulletinPaieTA.setRetenuePatron(bulletin.getTa());
				listImprimBulletinPaie.add(imprimBulletinPaieTA);
				...
				ImprimBulletinPaie imprimBulletinPaieFDFP= new ImprimBulletinPaie();
				imprimBulletinPaieFDFP.setLibelle("FDFP -TAXE A LA FPC ");
				imprimBulletinPaieFDFP.setTaux(BigDecimal.valueOf(0.6));
				imprimBulletinPaieFDFP.setBase(bulletin.getBrutImposable());
				imprimBulletinPaieFDFP.setRetenuePatron(bulletin.getFpc());
				listImprimBulletinPaie.add(imprimBulletinPaieFDFP);
				...
				imprimBulletinPaieFDFPREG.setTaux(BigDecimal.valueOf(0.6));
