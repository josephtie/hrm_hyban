...
public class BulletinPaieServiceImpl implements BulletinPaieService {
    ...
    @Override
    public RapportBullDTO buildBulletinData(Long idCV) {
        BulletinPaie bulletin=new BulletinPaie();
        RapportBullDTO rapportBullDTO=new RapportBullDTO();
            ...
            if(bulletin.getId() != null) {
                ...
                for (Rubrique rubrique : listPrimeImp) {
                    ...
                    if (listprimepersonnel.size() > 0 && listprimepersonnel.get(0) != null) {
                        ...
                        if (listprimepersonnel.get(0).getPrime().getTaux() != null)
                            imprimBulletinPaieDivImp.setTaux(BigDecimal.valueOf(listprimepersonnel.get(0).getValeur()));
                        else
                ...
                for (Rubrique rubrique : listPrimeImp1) {
                    ...
                    if (listprimepersonnel1.size() > 0 && listprimepersonnel1.get(0) != null) {
                        ...
                        imprimBulletinPaieDivImp10.setBase(listprimepersonnel1.get(0).getMontant().subtract(listprimepersonnel1.get(0).getPrime().getMtExedent()));
                        imprimBulletinPaieDivImp10.setGain(listprimepersonnel1.get(0).getMontant().subtract(listprimepersonnel1.get(0).getPrime().getMtExedent()));
...
//			listImprimBulletinPaie.add(imprimBulletinPaieRESP);
                ...
                imprimBulletinPaieCnps.setTaux(BigDecimal.valueOf(6.30));
                ...
                imprimBulletinPaieAC.setTauxPatron(BigDecimal.valueOf(2.0));
                ...
                imprimBulletinPaieFDFPRegul.setTauxPatron(BigDecimal.valueOf(0.6D));
                ...
                rapportBullDTO.setImprimBulletinPaies(listImprimBulletinPaie);
            ...
            //    modelMap.addAttribute("montantcumulIts", Utils.formattingAmount(bulletin.getCumulIts()));
              ...
              //  headers.setContentDispositionFormData("filename", "Bulletin_Paie.pdf");
                rapportBullDTO.setBulletinPaie(bulletin);
                return rapportBullDTO;
            }
        } catch (Exception e) {
    ...
    private void persistGeneratedBulletins(Long idPeriode, List<LivreDePaie> livreRows) {
        if (idPeriode == null || livreRows == null || livreRows.isEmpty()) {
            return;
        }

        PeriodePaie periodePaie = periodePaieRepository.findById(idPeriode).orElse(null);
        if (periodePaie == null) {
            return;
        }

        List<BulletinPaie> bullList = new ArrayList<BulletinPaie>();
        for (LivreDePaie livre : livreRows) {
            if (livre == null || livre.getBullpaie() == null || livre.getPeriodePaie() == null) {
                continue;
            }
            if (!Objects.equals(livre.getPeriodePaie().getId(), idPeriode)) {
                continue;
            }
    ...
    /**
     * Enrichit une liste de bulletins avec leurs listes de primes (PrimePersonnel)
     * classées par etatImposition.  Les champs sont @Transient côté JPA,
     * donc cette méthode doit être appelée après chaque chargement depuis la base.
     */
    private void enrichBulletinsWithPrimeLists(List<BulletinPaie> bulletins, PeriodePaie periode) {
        if (bulletins == null || bulletins.isEmpty() || periode == null) return;

        List<PrimePersonnel> allPrimes = primePersonnelRepository.findByPeriodePaieId(periode.getId());
        Map<Long, List<PrimePersonnel>> primesByContract = allPrimes.stream()
                .filter(p -> p.getContratPersonnel() != null)
                .collect(Collectors.groupingBy(p -> p.getContratPersonnel().getId()));

        for (BulletinPaie bulletin : bulletins) {
            if (bulletin.getContratPersonnel() == null) continue;

            Long contractId = bulletin.getContratPersonnel().getId();
            List<PrimePersonnel> contractPrimes = primesByContract.getOrDefault(contractId, Collections.emptyList());

            List<PrimePersonnel> listIndemniteBrut = new ArrayList<>();
            List<PrimePersonnel> listIndemniteNonImp = new ArrayList<>();
            ...
            for (PrimePersonnel pp : contractPrimes) {
                if (pp.getPrime() == null) continue;
                int etat = pp.getPrime().getEtatImposition();
                switch (etat) {
                    case 1 -> listIndemniteBrut.add(pp);
                    case 2 -> listIndemniteNonImp.add(pp);
                    case 3 -> listIndemniteNonImp.add(pp);
                    case 4 -> listRetenueMutuelle.add(pp);
                    case 5 -> listGainsNet.add(pp);
                    case 6 -> listRetenueSociale.add(pp);
                }
            }

            bulletin.setListIndemniteBrut(listIndemniteBrut);
            bulletin.setListIndemniteNonImp(listIndemniteNonImp);
            bulletin.setListRetenueMutuellt(listRetenueMutuelle);
            bulletin.setListRetenueSociale(listRetenueSociale);
            bulletin.setListGainsNet(listGainsNet);
        }
    }
    ...
    public BulletinPaieDTO loadBulletinPaie(Pageable pageable,PeriodePaie maperiode) {
        ...
        List<BulletinPaie> bulletins = page.getContent();
        enrichBulletinsWithPrimeLists(bulletins, maperiode);
    ...
    public List<ImprimBulletinPaie> getlistImpData(BulletinPaie bulletin){
        List<ImprimBulletinPaie> listImprimBulletinPaie = new ArrayList<ImprimBulletinPaie>();
        BigDecimal taux = bulletin.getJourTravail();

        listImprimBulletinPaie.add(ajouterImprimBulletinPaie( "SALAIRE DE BASE CATEGORIEL", taux, bulletin.getSalairbase(), bulletin.getSalairbase(),BigDecimal.ZERO,null,null));
        listImprimBulletinPaie.add(ajouterImprimBulletinPaie( "SURSALAIRE", taux, bulletin.getSursalaire(), bulletin.getSursalaire(),BigDecimal.ZERO,null,null));
        listImprimBulletinPaie.add(ajouterImprimBulletinPaie("PRIME D'ANCIENNETE", BigDecimal.ZERO, bulletin.getPrimeanciennete(), bulletin.getPrimeanciennete(),BigDecimal.ZERO,null,null));
        listImprimBulletinPaie.add(ajouterImprimBulletinPaie( "INDEMNITE DE RESIDENCE", taux, bulletin.getIndemnitelogement(), bulletin.getIndemnitelogement(),BigDecimal.ZERO,null,null));
        listImprimBulletinPaie.add(ajouterImprimBulletinPaie("IND. DE TRANSPORT IMP", taux, bulletin.getIndemniteTransportImp(), bulletin.getIndemniteTransportImp(),BigDecimal.ZERO,null,null));
        listImprimBulletinPaie.add(ajouterImprimBulletinPaie( "PRIMES IMPOSABLES", taux, bulletin.getAutreIndemImposable(), bulletin.getAutreIndemImposable(),BigDecimal.ZERO,null,null));
        ...
        for(Rubrique rubrique : listPrimeImp){
            ...
            if(listprimepersonnel.size()>0 &&listprimepersonnel.get(0) !=null){
                if(listprimepersonnel.get(0).getPrime().getTaux()!=null)
                    listImprimBulletinPaie.add(ajouterImprimBulletinPaie(listprimepersonnel.get(0).getPrime().getLibelle(),BigDecimal.valueOf(listprimepersonnel.get(0).getValeur()),listprimepersonnel.get(0).getMontant(),listprimepersonnel.get(0).getMontant(),BigDecimal.ZERO,null,null));
                else
                    listImprimBulletinPaie.add(ajouterImprimBulletinPaie(listprimepersonnel.get(0).getPrime().getLibelle(),taux,listprimepersonnel.get(0).getMontant(),listprimepersonnel.get(0).getMontant(),BigDecimal.ZERO,null,null));
            }
        }
        ...
        for(Rubrique rubrique : listPrimeImp1){
            ...
            listprimepersonnel1=	primePersonnelRepository.findByContratPersonnelIdAndPeriodePaieIdAndPrimeId(bulletin.getContratPersonnel().getId(),bulletin.getPeriodePaie().getId(),rubrique.getId());
            if(listprimepersonnel1.size()>0 &&listprimepersonnel1.get(0) !=null){
                listImprimBulletinPaie.add(ajouterImprimBulletinPaie(listprimepersonnel1.get(0).getPrime().getLibelle(),taux,listprimepersonnel1.get(0).getMontant().subtract(listprimepersonnel1.get(0).getPrime().getMtExedent()),listprimepersonnel1.get(0).getMontant().subtract(listprimepersonnel1.get(0).getPrime().getMtExedent()),BigDecimal.ZERO,null,null));
            }
        }

        listImprimBulletinPaie.add(ajouterImprimBulletinPaie("***** Salaire Brut Imposable ******",BigDecimal.ZERO,BigDecimal.ZERO,bulletin.getBrutImposable(),BigDecimal.ZERO,null,null));
        listImprimBulletinPaie.add(ajouterImprimBulletinPaie("IND. DE TRANSPORT NON IMPOS*",BigDecimal.ZERO,BigDecimal.ZERO,bulletin.getIndemniteTransport(),BigDecimal.ZERO,null,null));
        ...
        for(Rubrique rubrique : listPrimeImpnon){
            ...
            listprimepersonnel=	primePersonnelRepository.findByContratPersonnelIdAndPeriodePaieIdAndPrimeId(bulletin.getContratPersonnel().getId(),bulletin.getPeriodePaie().getId(),rubrique.getId());
            if(listprimepersonnel.size()>0 &&listprimepersonnel.get(0) !=null){
                listImprimBulletinPaie.add(ajouterImprimBulletinPaie(listprimepersonnel.get(0).getPrime().getLibelle(),taux,listprimepersonnel.get(0).getMontant(),bulletin.getBrutImposable(),BigDecimal.ZERO ,null,null));
        ...
        listImprimBulletinPaie.add(ajouterImprimBulletinPaie("***** Salaire Brut Non Imposable ******",taux,bulletin.getBrutNonImposable(),bulletin.getBrutNonImposable(),BigDecimal.ZERO,null,null));
    ...
    private ImprimBulletinPaie ajouterImprimBulletinPaie(String libelle,
                                                         BigDecimal taux, BigDecimal base, BigDecimal gain, BigDecimal retenue, BigDecimal tauxPatron, BigDecimal retenuePatron) {
        ...
        imprimBulletinPaie.setGain((gain != null) ? gain : BigDecimal.ZERO);
        imprimBulletinPaie.setRetenue((retenue != null) ? retenue : BigDecimal.ZERO);
        imprimBulletinPaie.setTauxPatron((tauxPatron != null) ? tauxPatron : BigDecimal.ZERO);
        imprimBulletinPaie.setRetenuePatron((retenuePatron != null) ? retenuePatron : BigDecimal.ZERO);
    ...
    public LivreDePaieDTO  genererMois1(Pageable pageable, Long idPeriode) {
        ...
        for(Personnel person : personnelListTrt){
            ...
            if(ctratpersonnellz.getAncienneteInitial()!=0) {
                newancienete= BigDecimal.valueOf( anneesAnciennete).add(BigDecimal.valueOf(ctratpersonnellz.getAncienneteInitial()));
            }else{
                newancienete=BigDecimal.valueOf(anneesAnciennete);
            }
            ...
            BigDecimal somavsAcpte=BigDecimal.ZERO;BigDecimal somalios=BigDecimal.ZERO;
            if(monEchelonnment.size()>0){
                for(int k = 0; k < monEchelonnment.size(); k++){
                    ...
                    if(monEchelonnment.get(k).getPretPersonnel().getPret().getId()==2L){
                        ...
                        somavsAcpte=monEchelonnment.get(k).getMontant().add(somavsAcpte);
                    }
                    // PRET ALOES
                    if(monEchelonnment.get(k).getPretPersonnel().getPret().getId()==1L ||monEchelonnment.get(k).getPretPersonnel().getPret().getId()==3L ){
                        ...
                        monechel.setPaye(true);
                        monechel=echelonnementRepository.save(monechel);
                        somalios=monEchelonnment.get(k).getMontant().add(somalios);
                    }
                }
            }
            ...
            //		 livrePaiecal = new LivreDePaie(ctratpersonnellz.getPersonnel().getMatricule(),ctratpersonnellz.getPersonnel().getNom()+" "+ctratpersonnellz.getPersonnel().getPrenom(), ctratpersonnellz.getPersonnel().getNombrePart(), op, ctratpersonnellz.getCategorie().getSalaireDeBase(),500BigDecimal.ZERO, ctratpersonnellz.getIndemniteLogement(),BigDecimal.ZERO, BigDecimal.ZERO,ctratpersonnellz,null,periodePaieActif);
            ...
            int op;
            if (ancSafe.compareTo(BigDecimal.valueOf(2)) < 0) {
                op = 0;
            } else if (ancSafe.compareTo(BigDecimal.valueOf(25)) <= 0) {
                op = ancSafe.intValue(); // ⚠️ conversion
            } else {
                op = 25;
            }
            ...
            BigDecimal cumulIgr = BigDecimal.ZERO;
            BigDecimal cumulIts = BigDecimal.ZERO;
            BigDecimal cumulCn = BigDecimal.ZERO;
            BigDecimal cumulCnps = BigDecimal.ZERO;
            BigDecimal cumulRetenuNet = BigDecimal.ZERO;
...
// récupération des bulletins de la période en cours et antérieurs
            ...
            for (BulletinPaie bulletinPaie : bulletinPaieList) {
                ...
                cumulIgr = cumulIgr.add(safe(bulletinPaie.getIgr()));
                cumulIts = cumulIts.add(safe(bulletinPaie.getIts()));
                cumulCn  = cumulCn.add(safe(bulletinPaie.getCn()));
                cumulCnps = cumulCnps.add(safe(bulletinPaie.getCnps()));
                salaireNet = salaireNet.add(safe(bulletinPaie.getNetapayer()));
            }

            // Ajout du mois courant
            // Ajout du mois courant
            cumulIgr = cumulIgr.add(safe(livrePaiecalR.getIgr()));
            cumulIts = cumulIts.add(safe(livrePaiecalR.getIts()));
            cumulCn  = cumulCn.add(safe(livrePaiecalR.getCn()));
            cumulCnps = cumulCnps.add(safe(livrePaiecalR.getCnps()));
            salaireNet = salaireNet.add(safe(livrePaiecalR.getNetPayer()));
...
// Mise à jour des champs du bulletin
            ...
            //LivreDePaie livrePaie = new LivreDePaie(person.getMatricule(),person.getNom(), nbrepart, anc, ctratpersonnellz.getCategorie().getSalaireDeBase(),2000BigDecimal.ZERO, indemLogt, somavsAcpte, somalios);
    ...
    private BigDecimal safeSum(BigDecimal... values) {
        ...
        for (BigDecimal v : values) {
            result = result.add(safeBigDecimal(v));
        }
        return result;
    }
    ...
    public LivreDePaieDTO genererOptimiseMois1(Pageable pageable, Long idPeriode) {
        ...
        LivreDePaieDTO livreDEPaieList = new LivreDePaieDTO();
        // ⚠️ Liste LOCALE (pas le champ partagé `this.livredepaieList`) pour éviter
        //    les races entre appels concurrents (pagination, double-clic, etc.).
        List<LivreDePaie> livredepaieList = new ArrayList<LivreDePaie>();
        ...
        logger.info("📊 [DIAG] Personnel (statut=true, retraitEffect=false) : {} entrée(s)", personnelList.size());

        // 3️⃣ Précharger tous les contrats actifs
        List<ContratPersonnel> contrats = contratPersonnelRepository.findLastByStatutTrue();
        logger.info("📊 [DIAG] Contrats (findLastByStatutTrue)             : {} entrée(s)", contrats.size());

        // Décompte des contrats avec depart=true (utile pour comparer avec /api/personnels)
        long contratsAvecDepart = contrats.stream()
                .filter(c -> Boolean.TRUE.equals(c.getDepart()))
                .count();
        logger.info("📊 [DIAG] Dont contrats depart=true                    : {}", contratsAvecDepart);

// 4️⃣ Créer une Map des contrats uniquement pour le personnel actif
        // En cas de plusieurs contrats actifs pour un même personnel, on garde
        // celui dont l'id est le plus élevé (= le plus récent).
        ...
        logger.info("📊 [DIAG] Map contratParPersonnel (1 contrat / pers)   : {} entrée(s)", contratParPersonnel.size());

        // Personnels actifs SANS contrat correspondant -> exclus du livre de paie.
        long personnelsSansContrat = personnelList.stream()
                .filter(p -> !contratParPersonnel.containsKey(p.getId()))
                .count();
        logger.info("📊 [DIAG] Personnels sans contrat dans la map          : {}", personnelsSansContrat);
//        Map<Long, ContratPersonnel> contratParPersonnel = contrats.stream()
...
//				.collect(Collectors.toMap(c -> c.getPersonnel().getId(), c -> c));
        ...
        Map<Long, List<PrimePersonnel>> primesParContrat = primes.stream()
                .filter(p -> p.getContratPersonnel() != null)
                .collect(Collectors.groupingBy(p -> p.getContratPersonnel().getId()));
        Map<Long, List<PrimePersonnel>> primesParPersonnel = primes.stream()
                .filter(p -> p.getContratPersonnel() != null && p.getContratPersonnel().getPersonnel() != null)
...
//				.collect(Collectors.toMap(t -> t.getPersonnel().getId(), t -> t));
        ...
        for (Personnel person : personnelList) {
            ...
            // 🔹 Primes et retenues
            List<PrimePersonnel> primesPersContrat = primesParContrat.getOrDefault(contrat.getId(), Collections.emptyList());
            List<PrimePersonnel> primesPersPersonnel = primesParPersonnel.getOrDefault(person.getId(), Collections.emptyList());
            Map<Long, PrimePersonnel> primesFusionnees = new LinkedHashMap<Long, PrimePersonnel>();

            for (PrimePersonnel primePers : primesPersContrat) {
                if (primePers != null && primePers.getPrime() != null && primePers.getPrime().getId() != null) {
                    primesFusionnees.put(primePers.getPrime().getId(), primePers);
                }
            }
            for (PrimePersonnel primePers : primesPersPersonnel) {
                if (primePers != null && primePers.getPrime() != null && primePers.getPrime().getId() != null) {
                    Long primeId = primePers.getPrime().getId();
                    PrimePersonnel existante = primesFusionnees.get(primeId);
                    if (existante == null) {
                        primesFusionnees.put(primeId, primePers);
                    } else {
                        double montantExistant = existante.getMontant() != null ? existante.getMontant().doubleValue() : 0d;
                        double montantPersonnel = primePers.getMontant() != null ? primePers.getMontant().doubleValue() : 0d;
                        if (montantExistant <= 0d && montantPersonnel > 0d) {
                            primesFusionnees.put(primeId, primePers);
                        }
                    }
                }
            ...
            List<PrimePersonnel> primesPers = new ArrayList<PrimePersonnel>(primesFusionnees.values());
            if (primesPersContrat.size() != primesPers.size()) {
                logger.warn("[PRIMES] Fusion contrat+personnel pour {} (contratId={}) : {} prime(s) contrat, {} après fusion",
                        person.getMatricule(), contrat.getId(), primesPersContrat.size(), primesPers.size());
            }
            List<PrimePersonnel> listIndemniteBrut = new ArrayList<>();
            ...
            BigDecimal somAvance = BigDecimal.ZERO;
            BigDecimal somPretAlios = BigDecimal.ZERO;
            ...
            for (Echelonnement e : echelsPers) {
                ...
                BigDecimal montant = safe(e.getMontant());

                if (pretId == 2L) { // AVANCE
                    somAvance = somAvance.add(montant);
                    e.setPaye(true);

                } else if (pretId == 1L || pretId == 3L) { // PRET ALOES
                    somPretAlios = somPretAlios.add(montant);
                    e.setPaye(true);
                }
            }
...
//			BigDecimal somAvance = echelsPers.stream()
//					.filter(e -> e.getPretPersonnel().getPret().getId() == 2L)
//					.mapToBigDecimal(Echelonnement::getMontant).sum();
//			BigDecimal somPretAlios = echelsPers.stream()
...
//					.mapToBigDecimal(Echelonnement::getMontant).sum();
            ...
            if(countbull==0){
                ...
                livrePaiecalR = new LivreDePaie(contrat.getPersonnel().getMatricule(),contrat.getPersonnel().getNom()+" "+contrat.getPersonnel().getPrenom(),nbpart , op, contrat.getCategorie().getSalaireDeBase(),contrat.getSursalaire(), contrat.getIndemniteLogement(),BigDecimal.ZERO, BigDecimal.ZERO,contrat,null,periodePaieActif,listIndemniteBrut,listIndemniteNonBrut,listRetenueMutuelle,listGainsNet,listRetenueSociale);
            ...
            livrePaiecalR.setPeriodePaie(periodePaieActif);
            livrePaiecalR.setAutreImposable(livrePaiecalR.getAutreIndemImposable());
            livrePaiecalR.setCMUPatronal(livrePaiecalR.getCMUPatronal());
            livrePaiecalR.setCMUSalarial(livrePaiecalR.getCMUPatronal());
            livrePaiecalR.setContratPersonnel(contrat);
            livrePaiecalR.setTotalBrut(livrePaiecalR.getBrutTotal());

            // 🔹 Reporter les jours/heures de présence saisis dans TempEffectif
            //    (sinon jourTravail / temptravail restent null dans le bulletin).
            if (tpeff != null) {
                if (tpeff.getJourspresence() != null) {
                    livrePaiecalR.setJourTravail(BigDecimal.valueOf(tpeff.getJourspresence()));
                }
                if (tpeff.getHeurspresence() != null) {
                    livrePaiecalR.setTemptravail(BigDecimal.valueOf(tpeff.getHeurspresence()));
                }
            }
            ...
            List<BulletinPaie> bulletinsPers =
                    bulletinsParPersonnel.getOrDefault(person.getId(), Collections.emptyList());

            BigDecimal cumulIgr = bulletinsPers.stream()
                    .map(b -> safeBigDecimal(b.getIgr()))
                    .reduce(BigDecimal.ZERO, BigDecimal::add);

            BigDecimal cumulIts = bulletinsPers.stream()
                    .map(b -> safeBigDecimal(b.getIts()))
                    .reduce(BigDecimal.ZERO, BigDecimal::add);

            BigDecimal cumulCn = bulletinsPers.stream()
                    .map(b -> safeBigDecimal(b.getCn()))
                    .reduce(BigDecimal.ZERO, BigDecimal::add);

            BigDecimal cumulCnps = bulletinsPers.stream()
                    .map(b -> safeBigDecimal(b.getCnps()))
                    .reduce(BigDecimal.ZERO, BigDecimal::add);

            BigDecimal cumulNet = bulletinsPers.stream()
                    .map(b -> safeBigDecimal(b.getNetapayer()))
                    .reduce(BigDecimal.ZERO, BigDecimal::add);

            // 🔹 Ajouter le mois courant


            cumulIgr = cumulIgr.add(safeBigDecimal(livrePaiecalR.getIgr()));
            cumulIts = cumulIts.add(safeBigDecimal(livrePaiecalR.getIts()));
            cumulCn  = cumulCn.add(safeBigDecimal(livrePaiecalR.getCn()));
            cumulCnps = cumulCnps.add(safeBigDecimal(livrePaiecalR.getCnps()));
            cumulNet = cumulNet.add(safeBigDecimal(livrePaiecalR.getNetPayer()));
            BigDecimal cumulRetenuNet = safeSum(cumulIgr, cumulIts, cumulCn, cumulCnps);
            ...
            detailsBull.setTotalmassesalarial(livrePaiecalR.getTotalMasseSalariale());
            detailsBull.setRetenueSociiale(livrePaiecalR.getRetenueSociiale());
            detailsBull.setListIndemniteBrut(listIndemniteBrut);
            detailsBull.setListIndemniteNonImp(listIndemniteNonBrut);
            detailsBull.setListRetenueSociale(listRetenueSociale);
            detailsBull.setListRetenueMutuellt(listRetenueMutuelle);
            detailsBull.setListGainsNet(listGainsNet);
            detailsBull.setTotalbrut(livrePaiecalR.getTotalBrut());
            detailsBull.setCalculer(true);
            detailsBull.setCloture(false);
            detailsBull.setPeriodePaie(livrePaiecalR.getPeriodePaie());
        ...
        persistGeneratedBulletins(idPeriode, livredepaieList);
...
//                .collect(Collectors.toMap(t -> t.getEmployee().getId(), t -> t));
        ...
        for (Employee person : personnelList) {
            ...
            for (Echelonnement e : echelsPers) {
                long pretId = e.getPretPersonnel().getPret().getId();
                if (pretId == 2L) { // AVANCE
                    somAvance.add(e.getMontant());
                    e.setPaye(true);
                } else if (pretId == 1L || pretId == 3L) { // PRET ALOES
                    somPretAlios.add( e.getMontant());
                    e.setPaye(true);
                }
            }
            if (!echelsPers.isEmpty()) {
...
//            detailsBull.setPretAlios(somPretAlios);
            ...
            //   BigDecimal cumulNet = bulletinsPers.stream().mapToBigDecimal(b -> safeBigDecimal(b.getNetPayer())).sum();

            // 🔹 Ajouter le mois courant


            //   cumulNet += safeBigDecimal(livrePaiecalR.getNetPayer());
...
//        BigDecimal salaireCategorie = Optional.ofNullable(contrat.getCategorie())
...
////        }
//
//        BigDecimal salaireBase = Math.ceil(
//                salaireCategorie * joursPresence / JOURS_OUVRABLES_MOIS
//        );
//
//        livre.setSalaireBase(salaireBase);
//        BigDecimal sursalaire = Optional.ofNullable(contrat.getSursalaire()).orElse(BigDecimal.ZERO);
//
//        BigDecimal sarsalaire = Math.ceil(
//                sursalaire * joursPresence / JOURS_OUVRABLES_MOIS
//        );
//
//        livre.setSurSalaire(sarsalaire);
//
//        BigDecimal transport = Optional.ofNullable(contrat.getIndemniteTransport()).orElse(BigDecimal.ZERO);
//
//        BigDecimal transportImposable = Math.ceil(
...
//
//        // ================== BRUT & RETENUES ==================
//        BigDecimal brutImposable = calculerBrutImposable(livre);
//        BigDecimal totalRetenue = calculerTotalRetenue(livre);
//
...
////        bull.setCMUPatronal(livre.getCarec() ? livre.getCMUPatronal() : BigDecimal.ZERO);
////        bull.setCMUSalarial(livre.getCarec() ? livre.getCMUSalarial() : BigDecimal.ZERO);
//
//        // 🔹 Métadonnées
...
//
//        livre.setTotalRetenue(BigDecimal.ZERO);
//        //livre.setTotalEchelonnement(BigDecimal.ZERO);
//        livre.setTotalPatronal(BigDecimal.ZERO);
//        livre.setNetPayer(0);
//    }
//    private void calculerChargesPatronales(LivreDePaieV2 livre) {
//
//        BigDecimal base = livre.getBrutImposable();
//
...
//
//        BigDecimal base = livre.getBrutImposable(); // ⚠️ UNIQUEMENT imposable
//
//        livre.setCnpsSalariale(Math.ceil(calculCNPS(base)));
//        BigDecimal ricf = getRICF(livre.getNombrePart());
//        BigDecimal itsbrut =Math.ceil(calculerITS(base,true));
//        livre.setIts(Math.max(0, itsbrut - ricf / 12));
//
//    }
//    private BigDecimal calculerBrutNonImposable(LivreDePaieV2 livre) {
//
//        BigDecimal total = 0;
//
//        for (PrimePersonnel p : livre.getIndemniteNonBrut()) {
//            total += safeBigDecimal(p.getMontant());
//        }
...
//    }
    ...
    //    private BigDecimal calculerBrutImposable(LivreDePaieV2 livre) {
...
//                    .stream()
//                    .mapToBigDecimal(PrimePersonnel::getMontant)
//                    .sum();
//        }
//
//        return Math.ceil(brut);
//    }
//    private BigDecimal calculerTotalRetenue(LivreDePaieV2 livre) {
//        return safeBigDecimal(livre.getIts())
//                + safeBigDecimal(livre.getCnpsSalariale())
//                + sommePrimes(livre.getRetenueMutuelle());
//    }
//    private BigDecimal sommeIndemnites(LivreDePaieV2 livre) {
...
//
//        BigDecimal cumulNet = prec.stream().mapToBigDecimal(b -> safeBigDecimal(b.getNetapayer())).sum();
//        cumulNet += safeBigDecimal(livre.getNetPayer());
//
//        livre.setCumulNet(cumulNet);
//    }
//    private void calculerCumuls(List<BulletinPaie> prec, LivreDePaieV2 livre) {
//
//        BigDecimal cumulNet = BigDecimal.ZERO;
//        BigDecimal cumulIts = BigDecimal.ZERO;
//        BigDecimal cumulCnpsSalariale = BigDecimal.ZERO;
//        BigDecimal cumulBrutImposable = BigDecimal.ZERO;
//        BigDecimal cumulCnpsPatronale= BigDecimal.ZERO;
//
...
//                    .mapToBigDecimal(b -> safeBigDecimal(b.getNetapayer()))
//                    .sum();
//
//            cumulIts = prec.stream()
//                    .mapToBigDecimal(b -> safeBigDecimal(b.getIts()))
//                    .sum();
//
//            cumulCnpsSalariale = prec.stream()
//                    .mapToBigDecimal(b -> safeBigDecimal(b.getCnps()))
//                    .sum();
//
//            cumulBrutImposable = prec.stream()
//                    .mapToBigDecimal(b -> safeBigDecimal(b.getBrutImposable()))
//                    .sum();
//
...
//        // 🔹 Ajouter le mois courant
//        cumulNet += safeBigDecimal(livre.getNetPayer());
//        cumulIts += safeBigDecimal(livre.getIts());
//        cumulCnpsSalariale += safeBigDecimal(livre.getCnpsSalariale());
//        cumulBrutImposable+= safeBigDecimal(livre.getBrutImposable());
...
//
//
//    private BigDecimal sommePrimes(List<PrimePersonnel> primes) {
//
...
//
//            BigDecimal montant = pp.getMontant();
//            BigDecimal valeur = pp.getValeur() != null ? pp.getValeur() : 1D;
//
//            // 🔹 Prime avec taux (%)
...
//    }
    ...
    public int countnbreJrdu(Date dateRetourDernierConge, Date dateDepartConge, ContratPersonnel Contratp) {
        ...
        BigDecimal ancSafe = BigDecimal.valueOf(anc) ;
        ...
        if(Contratp.getPersonnel().getSexe().equals("Feminin") && age>21 && Contratp.getPersonnel().getNombrEnfant()>0){
            ...
            if(Contratp.getPersonnel().getNombrEnfant()>=4)jourSuppDam=2;
...
//    }
    ...
    public  BigDecimal[] contratPersonnel(BigDecimal salaireCategoriel, Date dateEntree){
        ...
        BigDecimal partieEntiere = BigDecimal.valueOf(age) ;
        BigDecimal partieApresVirg = (BigDecimal.valueOf(age).subtract(partieEntiere).divide(BigDecimal.valueOf(12) ));


        if(age>=2)
            anciennete = (BigDecimal) (salaireCategoriel.multiply(partieEntiere).divide(BigDecimal.valueOf(100)));
...
//	public BigDecimal calculCNPS(BigDecimal brutImposable,BigDecimal indemniteRepresentation){
//		BigDecimal cnps = (brutImposable + indemniteRepresentation);
//		if(cnps < 1647315.0)
...
//       // pageImpianto=new PageImpl<BulletinSpeciale>(bullList.subList(start, end), pageable,bullList.size());
//
//        bulletinDTO.setRows(bullList);//pageImpianto.getContent());
//        bulletinDTO.setTotal(bullList.size());//pageImpianto.getTotalElements());
//
//
//        bulletinDTO.setResult("Bulletin generes avec succes");
//        return bulletinDTO.getRows();
//    }
    ...
    public BigDecimal salaireMoyenMensuel(ContratPersonnel contratPersonnel) {
        // TODO Auto-generated method stub
        BigDecimal salairMoyenMensuelle = BigDecimal.ZERO;
        List<BulletinPaie> bulletinPaieList = bulletinPaieRepository.findTop12ByContratPersonnel(contratPersonnel);
        for(BulletinPaie bulletinPaie : bulletinPaieList){
            salairMoyenMensuelle = bulletinPaie.getTotalbrut().add(bulletinPaie.getIndemniteTransport()).add(salairMoyenMensuelle)   ;
        }

        salairMoyenMensuelle = salairMoyenMensuelle.divide(BigDecimal.valueOf(bulletinPaieList.size()));
    ...
    public BigDecimal indemniteMoyenMensuel(ContratPersonnel contratPersonnel) {
        ...
        List<BulletinPaie> bulletinPaieList = bulletinPaieRepository.findTop12ByContratPersonnel(contratPersonnel);
        for(BulletinPaie bulletinPaie : bulletinPaieList){
            indemniteRepresentationMoyenMensuelle = bulletinPaie.getIndemniteRepresentation().add(bulletinPaie.getAutreNonImposable()).add(indemniteRepresentationMoyenMensuelle);
        }
        indemniteRepresentationMoyenMensuelle = indemniteRepresentationMoyenMensuelle.add(BigDecimal.valueOf(bulletinPaieList.size()) );
        return indemniteRepresentationMoyenMensuelle;
    }


    @Override
    public BigDecimal MensuelCn(PeriodePaie maperiode) {		// TODO Auto-generated method stub

        BigDecimal MensuelCn = BigDecimal.ZERO;
        List<BulletinPaie> bulletinPaieList = bulletinPaieRepository.findByPeriodePaie(maperiode);
        for(BulletinPaie bulletinPaie : bulletinPaieList){
            MensuelCn = bulletinPaie.getCn().add(MensuelCn) ;
        }
    ...
    public BigDecimal MensuelIgr(PeriodePaie maperiode) {
        // TODO Auto-generated method stub
        BigDecimal MensuelIgr = BigDecimal.ZERO;
        List<BulletinPaie> bulletinPaieList = bulletinPaieRepository.findByPeriodePaie(maperiode);
        for(BulletinPaie bulletinPaie : bulletinPaieList){
            MensuelIgr = bulletinPaie.getIgr().add(MensuelIgr) ;
        }
    ...
    public BigDecimal MensuelIs(PeriodePaie maperiode) {
        // TODO Auto-generated method stub
        BigDecimal MensuelIs = BigDecimal.ZERO;
        List<BulletinPaie> bulletinPaieList = bulletinPaieRepository.findByPeriodePaie(maperiode);
        for(BulletinPaie bulletinPaie : bulletinPaieList){
            MensuelIs = bulletinPaie.getIts().add(MensuelIs) ;
        }
    ...
    public BigDecimal MensuelIgrPatron(PeriodePaie maperiode) {
        // TODO Auto-generated method stub
        BigDecimal MensuelIgrPatron = BigDecimal.ZERO;
        List<BulletinPaie> bulletinPaieList = bulletinPaieRepository.findByPeriodePaie(maperiode);
        for(BulletinPaie bulletinPaie : bulletinPaieList){
            MensuelIgrPatron = bulletinPaie.getImpotSalaire().add(MensuelIgrPatron) ;
        }
    ...
    public BigDecimal MensuelBrut(PeriodePaie maperiode) {
        ...
        List<BulletinPaie> bulletinPaieList = bulletinPaieRepository.findByPeriodePaie(maperiode);
        for(BulletinPaie bulletinPaie : bulletinPaieList){
            MensuelIgrPatron = bulletinPaie.getTotalbrut().add(MensuelIgrPatron) ;
        }
    ...
    public BigDecimal MensuelCnAnne(Long anneeId) {
        ...
        for(BulletinPaie bulletinPaie : bulletinPaieList){
            MensuelIgrPatron = bulletinPaie.getCn().add(MensuelIgrPatron) ;
        }
    ...
    public BigDecimal MensuelIgrAnne(Long anneeId) {
        ...
        List<BulletinPaie> bulletinPaieList = bulletinPaieRepository.findByPeriodePaieAnneeId(anneeId);
        for(BulletinPaie bulletinPaie : bulletinPaieList){
            MensuelIgrPatron = bulletinPaie.getIgr().add(MensuelIgrPatron) ;
        }
...
//    public BigDecimal[] MensuelBaseCnpsSup(PeriodePaie maperiode) {
//        // TODO Auto-generated method stub
//        BigDecimal[] tab = new BigDecimal[2];
//        BigDecimal countff = BigDecimal.ZERO;BigDecimal Mensuelbasecnps = BigDecimal.ZERO;
//        List<BulletinPaie> bulletinPaieList = bulletinPaieRepository.findByPeriodePaie(maperiode);
//        for(BulletinPaie bulletinPaie : bulletinPaieList){
...
//    }
    ...
    public BigDecimal[] MensuelBaseCnpsSup70000(PeriodePaie maperiode) {
        ...
        List<BulletinPaie> bulletinPaieList = bulletinPaieRepository.findByPeriodePaie(maperiode);
        for(BulletinPaie bulletinPaie : bulletinPaieList){
            if(bulletinPaie.getBasecnps().compareTo(BigDecimal.valueOf(700000)) > 0 && bulletinPaie.getBasecnps().compareTo(BigDecimal.valueOf(3375000)) < 0 ){
                countff=countff.add(BigDecimal.valueOf(1));
                Mensuelbasecnps = bulletinPaie.getBasecnps().add(Mensuelbasecnps) ;
            }
        }
        tab[0] = countff;
        tab[1] = Mensuelbasecnps;
    ...
    public BigDecimal MensuelIsAnne(Long anneeId) {
        ...
        List<BulletinPaie> bulletinPaieList = bulletinPaieRepository.findByPeriodePaieAnneeId(anneeId);
        for(BulletinPaie bulletinPaie : bulletinPaieList){
            MensuelIgrPatron = bulletinPaie.getIts().add(MensuelIgrPatron);
        }
    ...
    public BigDecimal[] MensuelBaseCnpsInf(PeriodePaie periodePaie) {
        ...
        BigDecimal countff = BigDecimal.ZERO;
        BigDecimal mensuelBaseCnps = BigDecimal.ZERO;

        List<BulletinPaie> bulletinPaieList =
                bulletinPaieRepository.findByPeriodePaie(periodePaie);

        BigDecimal ZERO = BigDecimal.ZERO;
        BigDecimal PLAFOND = new BigDecimal("700000");

        for (BulletinPaie b : bulletinPaieList) {

            BigDecimal base = safe(b.getBasecnps());

            if (base.compareTo(ZERO) > 0 && base.compareTo(PLAFOND) < 0) {

                countff = countff.add(BigDecimal.ONE);
                mensuelBaseCnps = mensuelBaseCnps.add(base);
            }
        }
    ...
    public BigDecimal MensuelBrutImpAnne(Long anneeId) {
        ...
        List<BulletinPaie> bulletinPaieList = bulletinPaieRepository.findByPeriodePaieAnneeId(anneeId);
        for(BulletinPaie bulletinPaie : bulletinPaieList){
            MensuelIgrPatron = bulletinPaie.getBrutImposable().add(MensuelIgrPatron) ;
        }
    ...
    public BigDecimal MensuelJourtravail(PeriodePaie maperiode) {
        ...
        List<BulletinPaie> bulletinPaieList = bulletinPaieRepository.findByPeriodePaie(maperiode);
        for(BulletinPaie bulletinPaie : bulletinPaieList){
            MensuelIgrPatron = bulletinPaie.getJourTravail().add(MensuelIgrPatron) ;
        }
    ...
    public BigDecimal[] MasseSalarialdeLexo(PeriodePaie maperiode) {
        ...
        List<BulletinPaie> list = bulletinPaieRepository
                .findByPeriodePaieAnneeIdAndCalculer(
                        maperiode.getAnnee().getId(), true
                );

        for (BulletinPaie b : list) {

            if (b == null || b.getContratPersonnel() == null
                    || b.getContratPersonnel().getTypeContrat() == null) {
                continue;
            }

            Long typeId = b.getContratPersonnel().getTypeContrat().getId();

            if (typeId == 1L || typeId == 2L) {

                countff = countff.add(BigDecimal.ONE);
                masseSalariale = masseSalariale.add(
                        safe(b.getTotalmassesalarial())
                );
            }
        }
    ...
    public BigDecimal MasseSalarialMois(PeriodePaie maperiode) {
        ...
        return bulletinPaieRepository
                .findByPeriodePaieIdAndCalculer(maperiode.getId() - 1L, true)
                .stream()
                .map(b -> safe(b.getTotalmassesalarial()))
                .reduce(BigDecimal.ZERO, BigDecimal::add);
    }
