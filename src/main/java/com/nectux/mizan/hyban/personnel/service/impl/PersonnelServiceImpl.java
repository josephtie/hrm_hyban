...
public class PersonnelServiceImpl implements PersonnelService {
	...
	public PersonnelDTO findAllfilter(Map<String, String> filters, Pageable pageable) {
		...
		// Extraire la recherche textuelle et les filtres séparément
		String search = filters.get("search");
		String statutFilter = filters.get("statut");
		
		// Créer une nouvelle map sans le champ "search" et "statut" pour les filtres JPA textuels
		Map<String, String> jpaFilters = new HashMap<>(filters);
		jpaFilters.remove("search");
		jpaFilters.remove("statut");
		...
		for (Map.Entry<String, String> entry : jpaFilters.entrySet()) {
			String key = entry.getKey();
			String value = entry.getValue();
			
			if (value != null && !value.trim().isEmpty()) {
				Specification<Personnel> filterSpec = (root, query, criteriaBuilder) -> {
